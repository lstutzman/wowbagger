...
import { agingBucketChart, forecastChart, weeklyFlowChart } from './report-svg.js';
...
function renderEvidence(evidence, asOf) {
  const bucketChart = agingBucketChart(evidence.agingBuckets);
  const buckets = `${bucketChart === '' ? '' : `<figure class="chart-figure">${bucketChart}</figure>`}<table><thead><tr><th>Age</th><th>Open items</th></tr></thead><tbody>${evidence.agingBuckets.map((bucket) => `<tr><td>${escapeHtml(bucket.label)}</td><td>${bucket.count}</td></tr>`).join('')}</tbody></table>`;
  const flow = `<div class="table-scroll"><table><thead><tr><th>Week</th><th>Arrivals</th><th>Completions</th><th>Net</th></tr></thead><tbody>${evidence.weeks.map((week) => `<tr><td>${escapeHtml(week.weekStart)}</td><td>${week.arrivals}</td><td>${week.completions}</td><td>${week.arrivals - week.completions > 0 ? '+' : ''}${week.arrivals - week.completions}</td></tr>`).join('')}</tbody></table></div>`;
  ...
  const forecast = evidence.forecast === null
    ...
    : `<p><strong>${evidence.forecast.remaining}</strong> open items remaining.<br>50% by <strong>${escapeHtml(evidence.forecast.date50)}</strong> (${evidence.forecast.weeks50} weeks)<br>85% by <strong>${escapeHtml(evidence.forecast.date85)}</strong> (${evidence.forecast.weeks85} weeks)<br><span class="muted">${evidence.forecast.trials} Monte Carlo trials resampling observed weekly throughput.</span></p>`;

  const flowChart = weeklyFlowChart(evidence.weeks);
  const flowFigure = flowChart === ''
    ? ''
    : `<figure class="chart-figure">${flowChart}<figcaption><span class="key key-arrival">Arrivals</span><span class="key key-completion">Completions</span></figcaption></figure>`;
  const forecastBand = forecastChart(evidence.forecast, asOf);
  const forecastRow = forecastBand === ''
    ? ''
    : `<section class="chart-row"><h3>Forecast band</h3><figure class="chart-figure">${forecastBand}<figcaption><span class="key key-50">50% of trials</span><span class="key key-85">85% of trials</span></figcaption></figure></section>`;

  return `<section id="evidence" class="panel"><div class="section-heading"><div><p class="eyebrow">Evidence</p><h2>Flow and forecast</h2></div><p class="muted">Reconstructed from item dates in this ledger. No stored snapshots.</p></div><div class="evidence-grid"><section><h3>Throughput</h3><p><strong>${evidence.throughput.perWeek}</strong> items/week<br><span class="muted">${evidence.throughput.total} completions over ${evidence.throughput.windowWeeks} weeks.</span></p></section><section><h3>Cycle time</h3>${cycle}</section><section><h3>Forecast</h3>${forecast}</section><section><h3>Aging buckets</h3>${buckets}</section></div>${forecastRow}<section class="chart-row"><h3>Arrivals against completions</h3>${flowFigure}${flow}</section></section>`;
}
