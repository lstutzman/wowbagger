...
function renderCard(item, index, numbersById) {
  ...
  return `<details class="card" id="${itemAnchor(item)}" data-item data-order="${index}" data-state="${escapeHtml(item.readiness.state)}" data-status="${escapeHtml(item.status)}" data-priority="${item.priority ?? ''}" data-created="${escapeHtml(item.created)}" data-title="${escapeHtml(item.title.toLocaleLowerCase('en-US'))}" data-fields="${escapeHtml(JSON.stringify(item.fields))}" data-search="${escapeHtml(searchText)}">
...
function reveal(id){const target=cards.find(card=>card.id===id);if(target===undefined)return false;search.value='';state='all';for(const node of document.querySelectorAll('[data-filter]'))node.classList.toggle('active',node.dataset.filter==='all');for(const filter of slotFilters)filter.value='';apply();target.open=true;renderBody(target);target.style.scrollMarginTop=revealOffset()+'px';target.scrollIntoView({behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'start'});target.querySelector('summary').focus({preventScroll:true});return true;}
