...
function flowArrivalIds(scoped,week){var win=flowWindow();if(!win)return [];return scoped.filter(function(item){return weekStart(item.created)===week&&item.created>=win.start&&item.created<=win.end;}).map(function(item){return item.id;});}
function flowClosureIds(scoped,week){var win=flowWindow();if(!win)return [];return scoped.filter(function(item){return item.terminalDate!==null&&item.terminalDate!==undefined&&weekStart(item.terminalDate)===week&&item.terminalDate>=win.start&&item.terminalDate<=win.end;}).map(function(item){return item.id;});}
function flowDoneIds(scoped,week){var win=flowWindow();if(!win)return [];return scoped.filter(function(item){return item.status==='done'&&item.terminalDate!==null&&item.terminalDate!==undefined&&weekStart(item.terminalDate)===week&&item.terminalDate>=win.start&&item.terminalDate<=win.end;}).map(function(item){return item.id;});}
