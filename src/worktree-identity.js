...
// name it. Creation writes a fresh file in the same directory, fsyncs it, and
// renames it over the final path, so a reader on any interleaving sees either
// no file or one whole ID. The final path is never opened for writing, so a
// crash can never leave a truncated identity behind.
export async function ensureWorktreeIdentity({ ledgerDirectory, gitCommonDir }) {
  ...
  // Read back rather than trust the value just written: the caller must act on
  // the ID the next reader will see, not the one this process intended.
