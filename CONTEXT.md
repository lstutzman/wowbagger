...
- **host runtime** — the code-level `hostRuntime` parameter a process embedding an adapter entrypoint passes in: approval source, clock, redeemed-nonce store, and attested core executable identity; never a request member, and its absence is what makes a bare shipped adapter read-only.
