...
number: 192
title: "Scope report flow evidence and distinguish closures from completions"
kind: task
priority: 20
status: done
created: 2026-09-05
...
provenance:
  ...
  recorded_at: "2026-09-05T13:41:15.061Z"
depends_on: []
related: []
...
decisions:
  ...
  - action: complete
    date: 2026-09-05
    summary: "Added shared scoped evidence and SVG runtimes, accurate closure naming, inclusive event clipping, and exact range-duration rates."
    rationale: "Implementation commit 819b152; independent code review complete; all report regression tests pass in validation-033.txt. Evidence range boundary and rate defects reproduced before fixes. Final epic UI acceptance remains assigned to downstream items."
---

## Assignment

Implement T3: Scope report flow evidence and distinguish closures from completions. This is one standalone child of report epic #190. Scope and contracts are embedded here; the source plan is background, not a prerequisite for understanding the assignment. Parent ownership does not authorize taking another child's work.

## Required predecessors

None. This item can start independently once assigned.
...
- `src/report-evidence.js`
- `src/report-svg.js`
- `src/report-html.js`
- `test/report-evidence.test.js`
- `test/report-svg.test.js`
...
```sh
TMPDIR=/tmp /opt/homebrew/opt/node@24/bin/node --test test/report-evidence.test.js test/report-svg.test.js test/report-attention.test.js test/report-html.test.js
```
