...
Implement T5B: Connect report area concentrations and blocker drilldowns. This is one standalone child of report epic #190. Scope and contracts are embedded here; the source plan is background, not a prerequisite for understanding the assignment. Parent ownership does not authorize taking another child's work.

## Required predecessors

- #195 — Replace duplicated report lists with one item workspace.
- #193 — Derive report dependency impact independently of the UI.
...
- `src/report.js`
- `src/report-sequencing.js`
- `src/report-html.js`
- `test/report-model.test.js`
- `test/report-sequencing.test.js`
...
```sh
TMPDIR=/tmp /opt/homebrew/opt/node@24/bin/node --test test/report-model.test.js test/report-sequencing.test.js test/report-attention.test.js test/report-html.test.js
```
